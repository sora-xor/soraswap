...
- `repair_zero_supply_state(vault_account)`
...
Notes:
- Runtime config is singleton after init; `configure_hub` remains removed, but `bind_vault_account(...)` and `repair_zero_supply_state(...)` are retained as narrow owner-gated migration hooks for custody/accounting repair.
