...
- Public factory/product settlement verifies the configured oracle key, decodes the signed JSON privately, rejects degraded/stale/replayed slots, and uses `block_height()` as the current slot. Outperformance product settlement keeps `series_state()[1]` at the greater of the configured expiry and the verified settlement height. Controller-only `*_c2c` entrypoints carry already-verified numeric values because ABI v1 cannot yet construct JSON payloads with raw `bytes` fields for nested calls.
