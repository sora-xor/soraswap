import { KeyObject, generateKeyPairSync, sign, verify } from "crypto";
...
type OraclePayloadInput = Record<string, unknown>;
...
function compactPayload(input: OraclePayloadInput): Buffer {
  return Buffer.from(JSON.stringify(input), "utf8");
}

function signOracle(privateKey: KeyObject, input: OraclePayloadInput | Buffer): SignedOracle {
  const payload = Buffer.isBuffer(input) ? input : compactPayload(input);
  return {
    payload,
...
class SignedOracleVerifier {
  private readonly lastSlots = new Map<number, number>();

  constructor(
    private readonly publicKey: KeyObject,
    private readonly domain: number,
    private readonly subjectField: string,
    private readonly staleSlots: number,
    private readonly requiredPriceFields: string[] = []
  ) {}

  verifySubject(subjectId: number, signed: SignedOracle, currentBlockHeight: number): Record<string, number> {
    if (!verify(null, signed.payload, this.publicKey, signed.signature)) {
      throw new Error("oracle signature");
    }
    requirePositiveSafeInteger(subjectId, "oracle subject invalid");
    requireNonNegativeSafeInteger(currentBlockHeight, "invalid block height");
    const payload = this.decodePayload(signed.payload);
    const domain = this.readInt(payload, "domain");
    if (domain !== this.domain) {
      throw new Error("oracle domain mismatch");
    }
    const payloadSubjectId = this.readInt(payload, this.subjectField);
    if (payloadSubjectId <= 0) {
      throw new Error("oracle subject invalid");
    }
    if (payloadSubjectId !== subjectId) {
      throw new Error("oracle subject mismatch");
    }
    for (const field of this.requiredPriceFields) {
      const value = this.readInt(payload, field);
      if (value <= 0) {
        throw new Error(`oracle field ${field}`);
      }
    }
    const attestationHash = this.readInt(payload, "attestation_hash");
    if (attestationHash <= 0) {
      throw new Error("missing attestation");
    }
    const statusFlags = this.readInt(payload, "status_flags");
    if (statusFlags !== 0) {
      throw new Error("oracle degraded");
    }
    const oracleSlot = this.readInt(payload, "oracle_slot");
    if (currentBlockHeight < oracleSlot) {
      throw new Error("invalid oracle slot");
    }
    if (currentBlockHeight - oracleSlot > this.staleSlots) {
      throw new Error("oracle stale");
    }

    const priorSlot = this.lastSlots.get(subjectId) ?? 0;
    if (priorSlot > 0 && oracleSlot <= priorSlot) {
      throw new Error("oracle slot replay");
    }
    this.lastSlots.set(subjectId, oracleSlot);
  ...
  private decodePayload(payload: Buffer): Record<string, unknown> {
    ...
    try {
      ...
      throw new Error("oracle payload malformed");
    }
    if (!decoded || typeof decoded !== "object" || Array.isArray(decoded)) {
      throw new Error("oracle payload object");
    }
    return decoded as Record<string, unknown>;
  }

  private readInt(payload: Record<string, unknown>, field: string): number {
...
describe("Audit fix simulations", () => {
  test("signed oracle payloads reject tampering, stale slots, and replayed slots", () => {
    ...
    const verifier = new SignedOracleVerifier(publicKey, 1, "market_id", 4, [
      "mark_price_bps",
      "index_price_bps",
      "confidence_bps"
    ]);
    const good = signOracle(privateKey, {
      domain: 1,
      market_id: 7,
      mark_price_bps: 10_200,
      index_price_bps: 10_000,
      confidence_bps: 50,
      oracle_slot: 100,
      status_flags: 0,
      attestation_hash: 9001
    });

    const forgedPayload = compactPayload({
      domain: 1,
      market_id: 7,
      mark_price_bps: 1,
      index_price_bps: 10_000,
    ...
    expect(() => verifier.verifySubject(7, { payload: forgedPayload, signature: good.signature }, 100)).toThrow("oracle signature");

    const stale = signOracle(privateKey, {
      domain: 1,
      market_id: 7,
      mark_price_bps: 10_200,
      index_price_bps: 10_000,
      ...
      oracle_slot: 90,
      status_flags: 0,
      attestation_hash: 9002
    });
    expect(() => verifier.verifySubject(7, stale, 100)).toThrow("oracle stale");

    expect(verifier.verifySubject(7, good, 100).mark_price_bps).toBe(10_200);
    const replay = signOracle(privateKey, {
      domain: 1,
      market_id: 7,
      mark_price_bps: 10_250,
      index_price_bps: 10_000,
      confidence_bps: 40,
      oracle_slot: 100,
      status_flags: 0,
      attestation_hash: 9003
    });
    expect(() => verifier.verifySubject(7, replay, 101)).toThrow("oracle slot replay");
  });

  test("signed oracle payloads fail closed on malformed fields, wrong signer, and exact-byte replay attempts", () => {
    const { publicKey, privateKey } = generateKeyPairSync("ed25519");
    ...
    expect(() => verifier.verifySubject(7, signOracle(otherPrivateKey, base), 20)).toThrow("oracle signature");

    const signedBase = signOracle(privateKey, base);
    const sameFieldsDifferentBytes = Buffer.from(JSON.stringify({ ...base, confidence_bps: 50 }, null, 2), "utf8");
    expect(() =>
      verifier.verifySubject(7, { payload: sameFieldsDifferentBytes, signature: signedBase.signature }, 20)
    ).toThrow("oracle signature");

    expect(() => verifier.verifySubject(7, signOracle(privateKey, Buffer.from("{", "utf8")), 20)).toThrow(
      "oracle payload malformed"
    );
    expect(() => verifier.verifySubject(7, signOracle(privateKey, Buffer.from("[1,2]", "utf8")), 20)).toThrow(
      "oracle payload object"
    );

    const adversarialCases: Array<[string, OraclePayloadInput, string]> = [
      ["wrong domain", { ...base, domain: 2, oracle_slot: 21, attestation_hash: 9002 }, "oracle domain mismatch"],
      ["wrong subject", { ...base, market_id: 8, oracle_slot: 21, attestation_hash: 9003 }, "oracle subject mismatch"],
      ["degraded status", { ...base, oracle_slot: 21, status_flags: 1, attestation_hash: 9004 }, "oracle degraded"],
      ["future slot", { ...base, oracle_slot: 25, attestation_hash: 9005 }, "invalid oracle slot"],
      ["missing attestation", { ...base, oracle_slot: 21, attestation_hash: 0 }, "missing attestation"],
      ["missing mark", { ...base, oracle_slot: 21, mark_price_bps: undefined }, "oracle field mark_price_bps"],
      ["string mark", { ...base, oracle_slot: 21, mark_price_bps: "10200" }, "oracle field mark_price_bps"],
      ["fractional slot", { ...base, oracle_slot: 21.5, attestation_hash: 9006 }, "oracle field oracle_slot"]
    ];

    for (const [, payload, message] of adversarialCases) {
      expect(() => verifier.verifySubject(7, signOracle(privateKey, payload), 21)).toThrow(message);
    }

    expect(verifier.verifySubject(7, signedBase, 20).mark_price_bps).toBe(10_200);
    expect(
      verifier.verifySubject(
        8,
        signOracle(privateKey, { ...base, market_id: 8, oracle_slot: 20, attestation_hash: 9010 }),
        20
      ).mark_price_bps
    ).toBe(10_200);
    expect(() =>
      verifier.verifySubject(7, signOracle(privateKey, { ...base, oracle_slot: 19, attestation_hash: 9011 }), 20)
    ).toThrow("oracle slot replay");
  });

  test("oracle freshness boundaries and failed checks do not poison monotonic slot state", () => {
    const { publicKey, privateKey } = generateKeyPairSync("ed25519");
    const verifier = new SignedOracleVerifier(publicKey, 3, "policy_id", 4, ["observed_price"]);
    const base = {
      domain: 3,
      policy_id: 42,
      ...
      oracle_slot: 96,
      status_flags: 0,
      attestation_hash: 7001
    };

    expect(verifier.verifySubject(42, signOracle(privateKey, base), 100).oracle_slot).toBe(96);
    expect(() =>
      verifier.verifySubject(42, signOracle(privateKey, { ...base, oracle_slot: 97, status_flags: 1 }), 101)
    ).toThrow("oracle degraded");
    expect(verifier.verifySubject(42, signOracle(privateKey, { ...base, oracle_slot: 97, attestation_hash: 7002 }), 101).oracle_slot).toBe(97);
    expect(() =>
      verifier.verifySubject(42, signOracle(privateKey, { ...base, oracle_slot: 96, attestation_hash: 7003 }), 101)
    ).toThrow("oracle stale");
    expect(() =>
      verifier.verifySubject(42, signOracle(privateKey, { ...base, oracle_slot: 97, attestation_hash: 7004 }), 101)
    ).toThrow("oracle slot replay");
    expect(verifier.verifySubject(42, signOracle(privateKey, { ...base, oracle_slot: 98, attestation_hash: 7005 }), 102).oracle_slot).toBe(98);
  });

  test("oracle attestations cannot be replayed across product domains or subject fields", () => {
    const { publicKey, privateKey } = generateKeyPairSync("ed25519");
    const perpsVerifier = new SignedOracleVerifier(publicKey, 1, "market_id", 4, ["mark_price_bps"]);
    const optionsVerifier = new SignedOracleVerifier(publicKey, 2, "series_id", 4, ["final_mark_bps"]);
    const coverVerifier = new SignedOracleVerifier(publicKey, 3, "policy_id", 4, ["observed_price"]);

    const perpsPayload = signOracle(privateKey, {
      domain: 1,
      market_id: 7,
      series_id: 7,
      policy_id: 7,
      mark_price_bps: 10_200,
      final_mark_bps: 10_200,
      observed_price: 10_200,
      oracle_slot: 30,
      status_flags: 0,
      attestation_hash: 5001
    });
    expect(perpsVerifier.verifySubject(7, perpsPayload, 30).mark_price_bps).toBe(10_200);
    expect(() => optionsVerifier.verifySubject(7, perpsPayload, 30)).toThrow("oracle domain mismatch");
    expect(() => coverVerifier.verifySubject(7, perpsPayload, 30)).toThrow("oracle domain mismatch");

    const wrongSubjectField = signOracle(privateKey, {
      domain: 2,
      market_id: 7,
      final_mark_bps: 10_200,
      oracle_slot: 30,
      status_flags: 0,
      attestation_hash: 5002
    });
    expect(() => optionsVerifier.verifySubject(7, wrongSubjectField, 30)).toThrow("oracle field series_id");

    const wrongSubjectId = signOracle(privateKey, {
      domain: 3,
      policy_id: 8,
      observed_price: 10_200,
      oracle_slot: 30,
      status_flags: 0,
      attestation_hash: 5003
    });
    expect(() => coverVerifier.verifySubject(7, wrongSubjectId, 30)).toThrow("oracle subject mismatch");
  });

  test("oracle rejects unsafe integers and failed attestations do not advance slots", () => {
    const { publicKey, privateKey } = generateKeyPairSync("ed25519");
    const verifier = new SignedOracleVerifier(publicKey, 4, "asset_id", 2, ["price"]);
    const base = {
      domain: 4,
      asset_id: 99,
      price: 1_000,
      oracle_slot: 10,
      status_flags: 0,
      attestation_hash: 6001
    };

    expect(() => verifier.verifySubject(99, signOracle(privateKey, Buffer.from("null", "utf8")), 10)).toThrow(
      "oracle payload object"
    );
    expect(() =>
      verifier.verifySubject(99, signOracle(privateKey, { ...base, price: Number.MAX_SAFE_INTEGER + 1 }), 10)
    ).toThrow("oracle field price");
    expect(() =>
      verifier.verifySubject(99, signOracle(privateKey, { ...base, oracle_slot: 13, attestation_hash: 6002 }), 10)
    ).toThrow("invalid oracle slot");

    expect(verifier.verifySubject(99, signOracle(privateKey, base), 12).oracle_slot).toBe(10);
    expect(() =>
      verifier.verifySubject(99, signOracle(privateKey, { ...base, oracle_slot: 11, attestation_hash: 6003 }), 14)
    ).toThrow("oracle stale");
    expect(
      verifier.verifySubject(99, signOracle(privateKey, { ...base, oracle_slot: 11, attestation_hash: 6004 }), 13)
        .oracle_slot
    ).toBe(11);
  });

  test("oracle rejects duplicate-key shadowing and high-slot failures do not poison subjects", () => {
    const { publicKey, privateKey } = generateKeyPairSync("ed25519");
    const verifier = new SignedOracleVerifier(publicKey, 5, "market_id", 4, ["price"]);
    const duplicateDomain = Buffer.from(
      '{"domain":5,"domain":6,"market_id":1,"price":1000,"oracle_slot":1,"status_flags":0,"attestation_hash":1}',
      "utf8"
    );
    const duplicatePrice = Buffer.from(
      '{"domain":5,"market_id":1,"price":1000,"price":0,"oracle_slot":1,"status_flags":0,"attestation_hash":2}',
      "utf8"
    );

    expect(() => verifier.verifySubject(1, signOracle(privateKey, Buffer.alloc(0)), 1)).toThrow(
      "oracle payload malformed"
    );
    expect(() => verifier.verifySubject(1, signOracle(privateKey, duplicateDomain), 1)).toThrow(
      "oracle domain mismatch"
    );
    expect(() => verifier.verifySubject(1, signOracle(privateKey, duplicatePrice), 1)).toThrow("oracle field price");
    expect(() =>
      verifier.verifySubject(
        1,
        signOracle(privateKey, {
          domain: 6,
          market_id: 1,
          price: 1_000,
          oracle_slot: 99,
    ...
    ).toThrow("oracle domain mismatch");
    expect(
      verifier.verifySubject(
        1,
        signOracle(privateKey, {
  ...
  test("oracle rejects invalid subject ids and malformed block-height context", () => {
    const { publicKey, privateKey } = generateKeyPairSync("ed25519");
    const verifier = new SignedOracleVerifier(publicKey, 6, "market_id", 4, ["price"]);
    const valid = {
      domain: 6,
      market_id: 1,
      price: 1_000,
      oracle_slot: 1,
      status_flags: 0,
      attestation_hash: 6101
    };

    expect(() => verifier.verifySubject(0, signOracle(privateKey, { ...valid, market_id: 0 }), 1)).toThrow(
      "oracle subject invalid"
    );
    expect(() => verifier.verifySubject(-1, signOracle(privateKey, { ...valid, market_id: -1 }), 1)).toThrow(
      "oracle subject invalid"
    );
    expect(() => verifier.verifySubject(1.5, signOracle(privateKey, { ...valid, market_id: 1.5 }), 1)).toThrow(
      "oracle subject invalid"
    );
    expect(() => verifier.verifySubject(1, signOracle(privateKey, valid), -1)).toThrow("invalid block height");
    expect(() => verifier.verifySubject(1, signOracle(privateKey, valid), 1.5)).toThrow("invalid block height");
    expect(verifier.verifySubject(1, signOracle(privateKey, valid), 1).oracle_slot).toBe(1);
  });
